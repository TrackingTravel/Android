...
class RequestFragment : Fragment() {
    ...
    private fun onGotGeoPermissionResult(grantResults: Map<String, Boolean>) {
        if (grantResults.entries.all { it.value }) {
            ...
            if (!shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_FINE_LOCATION) &&
                ...
                Toast.makeText(activity, R.string.GPS_denied, Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun askUserForOpeningAppSettings() {
        ...
        if (activity?.getPackageManager()
            ...
            Toast.makeText(activity, R.string.GPS_denied_forever, Toast.LENGTH_SHORT).show()
        } else {
    ...
    private fun onGeoPermissionGranted() {
        (activity as MainActivity).startRouteFragment()
        Toast.makeText(activity, R.string.GPS_on, Toast.LENGTH_SHORT).show()
    }
